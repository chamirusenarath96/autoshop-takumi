...
- **New public page or admin view (standing requirement since the visual regression testing feature)** → also add visual snapshot coverage to `e2e/visual.spec.ts` (the `visual` Playwright project, `playwright.visual.config.ts`) so a blank/badly-broken render gets caught the way PR #14's incident should have been. Mask any genuinely run-varying region (timestamps, auto-generated IDs) via `getVisualMasks()` in `e2e/helpers.ts` rather than leaving it unmasked and flaky. New baselines only become authoritative once generated by the `update-visual-baselines` GitHub Actions workflow — see README's "Visual regression tests" section
