...
**GraphQL is not documented here — empirically verified non-functional (T032)**: `src/app/(payload)/api/[...slug]/route.ts` (this app's REST catch-all) exports only REST handlers, no `GRAPHQL_POST`; the only GraphQL-related route file, `src/app/(payload)/graphql/route.ts`, mounts solely `GRAPHQL_PLAYGROUND_GET` (the interactive query UI) with no POST handler for it to call. Per spec.md FR-009a, this was verified empirically against the running dev server rather than assumed from source: `POST /api/graphql` with a minimal query (`{"query":"{ Makes { docs { id } } }"}`) returns `HTTP 404 {"message":"Route not found \"/api/graphql\""}` (the REST catch-all does not map a GraphQL route), and no other route file exposes a GraphQL query POST handler. **GraphQL queries are therefore not reachable in this app** — a pre-existing condition, independent of this feature. Every GraphQL reference in spec.md (FR-007, FR-009, SC-004, Edge Cases, Assumptions) is consequently void/inapplicable and should be read as REST/Local API only, per FR-009a. Should a future change actually mount a GraphQL query handler, this contract doc should be extended with GraphQL request/response examples at that point, mirroring the REST examples below.
