...
**Scale/Scope**: Three collections (Makes, Models, Media) plus two globals (SiteSettings, Homepage); thirteen `localized: true` fields become twenty-six paired fields (data-model.md); four of those thirteen source fields are schema-`required: true` today and need an at-least-one-language validation replacement, not just an unconditional split (spec FR-013); every public/admin consumer of the old field names needs updating, most centrally `src/lib/site-settings.ts`'s `getSiteSettings(locale)`; one migration script covering five schemas, reading with `fallbackLocale: false` throughout (spec FR-014); one config removal (`payload.config.ts`) contingent on #19 AND on `Vehicles.gallery[].caption`'s unresolved out-of-scope status (spec Assumptions); no new entities or routes
