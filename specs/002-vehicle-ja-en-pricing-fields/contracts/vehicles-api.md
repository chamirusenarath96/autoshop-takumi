...
**Locale scope removed — for the nine migrated content fields only**: before this feature, reading/writing the nine in-scope content fields required a `?locale=` query param (REST) or `{ locale: 'ja' | 'en' }` option (Local API) to select which language's value was returned — omitting it defaulted to `ja` (the collection's `defaultLocale`). After this feature, every request reads/writes those nine fields explicitly by paired field name (`titleJa`, `titleEn`, etc.) — `?locale=`/`{ locale }` no longer has any effect on them. It is NOT locale-independent for the whole collection: `gallery[].caption` stays `localized: true` (out of scope, per spec.md Assumptions), so `?locale=`/`{ locale }` still resolves that one field exactly as before. It may also still matter for relationship lookups into still-localized collections like `Makes`/`Models`, which are unaffected by this feature.
...
```
Both languages and both currency fields are present in a single response, regardless of any `?locale=` param. These two pairs are read differently by the caller: for the **content** fields (`titleJa`/`titleEn`, etc.), the caller picks which one to display using the active site locale, with the fallback rule from data-model.md when one language is blank (spec FR-005/FR-006). For the **price** fields (`priceJpy`/`priceUsd`), the caller displays whichever is populated — both if both are set — identically regardless of site locale; there is no locale-based selection or fallback between them (spec FR-004).
...
**Scope of this contract**: this section describes the public vehicle listing page's own query construction (`src/app/(public)/[locale]/vehicles/page.tsx`, via the Local API), not a guarantee about the generic, framework-autogenerated `GET /api/vehicles` REST route's `sort`/`where` behavior in general. Payload does not expose any nulls-first/nulls-last control on its `sort` parameter — ordering of a `NULL` field value is left entirely to the underlying database adapter, with no supported way to override it through a `where`/`sort` query alone (confirmed against Payload's own sort documentation). Building a custom cross-database null-ordering guarantee (a computed sort field, a raw SQL view, or client-side re-sorting across paginated results) is explicitly **not** undertaken by this feature — it would be disproportionate engineering for what is expected to be a rare edge case (a listing priced in USD only), per Constitution Principle VI (no premature abstraction without concrete need).

**Missing-`priceJpy` behavior** (a listing with a USD price but no JPY price): such a listing is excluded from a `where[priceJpy]`-filtered result, since there is no JPY value to compare against the range. Its position under `sort=priceJpy`/`sort=-priceJpy` follows whichever `NULL`-ordering the active database applies natively — this is a **known, accepted difference** between local dev (SQLite) and production (Postgres), documented here rather than normalized away. The listing remains fully visible via normal pagination when no price filter/sort is applied, in both environments.
