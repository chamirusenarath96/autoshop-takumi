...
**Decision**: Sort and range-filter the vehicle listing by the `priceJpy` field only (asc/desc, `priceFrom`/`priceTo`), replacing the current single-`price`-field sort/filter (`where.price.greater_than_equal`/`less_than_equal`, `sort: 'price'`/`'-price'` in `src/app/(public)/[locale]/vehicles/page.tsx`), since JPY was already the collection's default/primary currency. A listing with only a USD price (no `priceJpy`) is: (a) excluded from a `priceFrom`/`priceTo`-filtered result, since there is no JPY value to compare against the range, and (b) sorted after every JPY-priced listing regardless of ascending/descending direction when `sort=price`/`sort=-price` is applied — i.e. treated as "unset," not as `0` or `Infinity`. Such a listing remains fully visible via normal browsing/pagination when no price filter or sort is applied.

**Rationale**: Comparing a JPY-denominated listing against a USD-denominated one numerically would be meaningless without a conversion this feature explicitly avoids introducing (see §2). Keeping JPY as the canonical sort/filter key requires no new sort logic beyond a field-name swap, and matches spec.md's documented Assumption (FR-012). Explicitly defining the missing-JPY behavior (rather than leaving it to each database's default null-ordering) matters because this repo runs SQLite locally and Postgres in production — the two engines do not order `NULL` the same way by default (SQLite: nulls first in `ASC`, last in `DESC`; Postgres: nulls last in `ASC`, first in `DESC`), so relying on either engine's native default would produce visibly different, database-dependent behavior. The implementation must apply an explicit, consistent rule (e.g. a `sort` value or query construction that forces nulls-last in both directions, or an equivalent explicit tie-break) rather than the bare column default, and this should be verified against both database backends during implementation, not assumed.

**Alternatives considered**: A conversion-based unified sort key was rejected as out of scope (requires the FX dependency explicitly deprioritized in design issue #18). A dual sort control (sort by JPY price vs. sort by USD price) was considered but adds UI surface with no requirement driving it in spec.md — can be a later feature if real demand appears. Excluding USD-only listings from the listing page entirely (rather than just from price-filtered/sorted views) was rejected as a regression — a USD-priced vehicle is still for sale and must remain browsable.
