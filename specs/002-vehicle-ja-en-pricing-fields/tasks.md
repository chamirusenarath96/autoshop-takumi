...
- [ ] T034 [P] `e2e/admin.spec.ts`: add a regression case asserting the admin vehicle list view renders correctly after T033/T036 — the title column shows a real, non-blank value (via the new fields' fallback) and no column references a removed field
- [ ] T035 Update `e2e/api.spec.ts` — it currently sends/asserts the legacy `title`, `price`, and `currency` fields directly (e.g. a vehicle-create request with `currency: 'JPY'`, assertions on `body.doc.title`/`body.doc.price`) — to use `titleJa`/`titleEn` and `priceJpy`/`priceUsd` instead, and update any assertions that depended on the old single-price/single-locale publish behavior to match the new publish-gate rule (data-model.md); this MUST land before T036 removes the fields this spec currently exercises
- [ ] T036 Remove the old localized fields (`title`, `exteriorColor`, `summary`, `highlights[].text`, `description`, `specs[].label`/`value`, `seoTitle`, `seoDescription` — NOT `gallery[].caption`, which stays out of scope) and the old `price`/`currency` fields from `src/collections/Vehicles.ts`, now that every consumer and test (T033, T035) no longer references them and the migration has been validated
- [ ] T037 [P] Regenerate `payload-types.ts` again after old-field removal and resolve any resulting type errors (`npx tsc --noEmit`)
- [ ] T038 Run the full suite (`npm test`, `npx tsc --noEmit`, `npm run test:e2e`) and confirm all green, per this repo's CLAUDE.md workflow
