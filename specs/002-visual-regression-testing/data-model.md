...
| Attribute | Type | Notes |
|---|---|---|
| `allure-results/` directory | Directory of Allure's intermediate JSON result files | Produced by the `allure-playwright` reporter on every run (local and CI), per FR-008. Cleared immediately before each run starts (the reporter appends rather than overwrites), so this directory always reflects exactly one run |
