...
**Testing**: Playwright (existing `e2e/*.spec.ts` suite: `admin.spec.ts`, `public.spec.ts`, `api.spec.ts`, `responsive.spec.ts`, run by the existing `e2e` CI job via `playwright.config.ts` — test definitions and `e2e/global-setup.ts` entirely unchanged by this feature); this feature adds `e2e/visual.spec.ts` and `e2e/visual-first-run.spec.ts` as new spec files, a new `playwright.visual.config.ts` config file (with `visual`/`visual-first-run` projects — deliberately **not** added to the existing `playwright.config.ts`, since that file's `globalSetup` would otherwise run before `visual-first-run` too and defeat its whole purpose; see research.md's Test-organization decision), and a new `e2e/visual-setup.ts` plain script (not a Playwright `globalSetup`) — all run by a new, separate `visual-e2e` CI job (see research.md's Isolation-model decision)
...
**Constraints**: Visual baselines MUST only be generated/compared in the CI Linux container (FR-006); genuinely run-varying content (timestamps, Payload's own auto-generated internal document IDs) MUST be masked, while the fixture's fixed, deterministic slug MUST NOT be masked since it's a useful stable assertion (FR-004, research.md's masking decision); adding visual coverage MUST NOT change any existing functional test's pass/fail behavior (FR-010, SC-006)
...
playwright.config.ts           # MODIFIED — `chromium` gets `testIgnore` for both new spec files (defense in depth); add allure-playwright reporter — `globalSetup`/`chromium`'s test definitions otherwise untouched

playwright.visual.config.ts    # NEW — second config file, no `globalSetup`, defining the `visual` project (testMatch: visual.spec.ts) and `visual-first-run` project (testMatch: visual-first-run.spec.ts); allure-playwright reporter; explicit expect.toHaveScreenshot tolerance; consistent colorScheme/deviceScaleFactor

.github/workflows/ci.yml       # MODIFIED — new `visual-e2e` job (own database, runs `visual-first-run` → visual-setup.ts → `visual`, both via `--config=playwright.visual.config.ts`, uploads its own allure-results); existing `e2e` job gets an unconditional (not gated on success) allure-results upload step; new `update-visual-baselines` workflow_dispatch job (contents: write, ubuntu-24.04) for regenerating visual baselines (see research.md)

package.json                   # MODIFIED — add allure-playwright, allure devDependencies; test:e2e stays bare `playwright test` (unchanged — implicitly uses playwright.config.ts, which has no visual projects to accidentally run); new test:e2e:visual (`playwright test --config=playwright.visual.config.ts --project=visual`) and allure:report scripts; pretest:e2e clears allure-results/ before each invocation (allure-playwright appends rather than overwriting)
...
**Structure Decision**: This is a test-infrastructure-only feature within the existing single Next.js + Payload monorepo (no frontend/backend split, no new app). All new spec/helper files live under the existing `e2e/` directory alongside current spec files, following the same flat-file-per-concern convention already established (`admin.spec.ts`, `public.spec.ts`, etc.) rather than introducing a new subdirectory structure. Playwright's own `project` mechanism satisfies FR-005's "distinct, separately runnable/reportable group" requirement without any custom tooling — but because Playwright's `globalSetup` is a single top-level, non-per-project config property, achieving a genuinely `globalSetup`-free project required a second config file (`playwright.visual.config.ts`) rather than a second `projects` entry in the existing one. A new dedicated CI job (`visual-e2e`, alongside the existing `e2e`/`test`/`typecheck`/`build` jobs) with its own database satisfies the data isolation this feature's snapshot determinism needs, while leaving `chromium`'s test definitions and `e2e/global-setup.ts` completely untouched.
