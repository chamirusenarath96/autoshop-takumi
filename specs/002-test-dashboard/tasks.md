...
- [ ] T008 [P] Implement a read-only R2 client wrapper in `src/lib/r2.ts` using `@aws-sdk/client-s3`, obtaining short-lived R2 **temporary credentials** scoped to `prefixes: ["testing-artifacts/"]` (minted server-side via R2's temporary-credentials endpoint using a long-lived management token, then used for all object reads) rather than a static long-lived token, since static R2 tokens only scope to a bucket, not a prefix (research.md §5)
...
- [ ] T011 [P] Create `.env.example` documenting `AUTH_GITHUB_ID`, `AUTH_GITHUB_SECRET`, `AUTH_SECRET`, `ALLOWED_DASHBOARD_GITHUB_LOGIN`, `ALLOWED_DASHBOARD_GITHUB_ID`, and the R2 configuration needed to mint temporary credentials — `R2_BUCKET`, `R2_ENDPOINT`, `R2_ACCOUNT_ID`, and a management-scoped `R2_TEMP_CREDENTIALS_TOKEN` used only server-side to request short-lived, `testing-artifacts/`-scoped credentials (naming otherwise matches this main repo's `resolveR2Config`/`deploymentConfig.ts` convention where applicable). `R2_PUBLIC_URL` is explicitly **not** used — report assets are proxied through the authorized session, not served from a public bucket URL (see `contracts/runs-data-contract.md`) (FR-010, FR-011)
...
- [ ] T034 Verify the dashboard's R2 temporary credentials with an **effective-policy test** against disposable validation objects (not the main app's real media): confirm a `GetObject`/`ListObjectsV2` call outside `testing-artifacts/` is denied, and that `PutObject`/`DeleteObject` are denied everywhere, including inside `testing-artifacts/` — "the config says read-only" is not sufficient without exercising the actual denial (FR-009, SC-004, research.md §5; quickstart.md Scenario 6)
