...
- [ ] T008 [P] Implement a read-only R2 client wrapper in `src/lib/r2.ts` using `@aws-sdk/client-s3`, authenticated against the **dedicated test-artifacts R2 bucket** (never the main app's production-media bucket) — a bucket-scoped static token is sufficient given the dedicated bucket contains nothing but test artifacts, since R2 temporary credentials can't exceed their parent token's permissions and would gain nothing extra here (research.md §5); optionally layer short-lived temporary credentials on top for defense-in-depth, per research.md's Credential lifecycle guidance, but this is not load-bearing for isolation the way it would be on a shared bucket
...
- [ ] T010 Implement `listRunPrefixes()` and `readRunSummary(runId)` in `src/lib/runs.ts` reading `testing-artifacts/<run-id>/summary.json` via `src/lib/r2.ts`, per `contracts/runs-data-contract.md` — deriving the expected `runId` from the R2 prefix itself (not trusting the JSON's own `runId` field), deriving `reportPath` from that prefix-derived `runId` and rejecting any `summary.json` whose `reportPath` doesn't match (or contains traversal/absolute-URL patterns), with keyset (`beforeRunId`) pagination per the contract's ordering rules, and treating a missing/malformed `summary.json` as `status: 'incomplete'` rather than throwing (depends on T008, T009)
- [ ] T011 [P] Create `.env.example` documenting `AUTH_GITHUB_ID`, `AUTH_GITHUB_SECRET`, `AUTH_SECRET`, `ALLOWED_DASHBOARD_GITHUB_LOGIN`, `ALLOWED_DASHBOARD_GITHUB_ID`, and the dedicated-bucket R2 credentials — `R2_TEST_ARTIFACTS_BUCKET`, `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` (a bucket-scoped static token for the dedicated test-artifacts bucket, distinctly named from this main repo's own `resolveR2Config`/`deploymentConfig.ts` variables to make clear it's a different bucket and credential entirely, not the shared-bucket variant an earlier draft of this spec assumed). `R2_PUBLIC_URL` is explicitly **not** used — report assets are proxied through the authorized session, not served from a public bucket URL (see `contracts/runs-data-contract.md`) (FR-010, FR-011)
...
- [ ] T034 Verify the dashboard's R2 credentials with an **effective-policy test** against disposable validation objects (not the main app's real media): confirm a `GetObjectCommand`/`ListObjectsV2Command` call against the main app's production-media bucket is denied entirely (proving the dedicated-bucket boundary actually holds, not just that it's configured that way), and that `PutObject`/`DeleteObject` are denied even within the dashboard's own dedicated bucket — "the config says read-only, different bucket" is not sufficient without exercising the actual denial (FR-009, SC-004, research.md §5; quickstart.md Scenario 6)
