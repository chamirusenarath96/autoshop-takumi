...
- What happens if the allowlisted GitHub account's login changes (e.g. the account is renamed)? Access is governed by GitHub's stable account identity, not just the login string, wherever the chosen OAuth implementation makes that distinction available — this is a configuration/rotation concern documented in the project's README, not a runtime feature.
...
- **FR-002**: The dashboard MUST reject sign-in for any GitHub account other than the single pre-configured allowed account, showing a clear access-denied state instead of any dashboard content.
...
- **Results storage**: The dashboard reads Allure artifacts from the same object storage bucket/service the main app already uses in production for media (a distinct prefix/namespace, not the media collection itself), rather than introducing a second storage provider, per the issue's own recommendation. The exact mechanism by which CI uploads artifacts into that storage (vs. the dashboard pulling them from GitHub Actions' own artifact API) is a technical decision deferred to the planning phase — both are compatible with FR-008.
